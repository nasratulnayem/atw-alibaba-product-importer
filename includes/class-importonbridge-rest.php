...
final class ImportonBridge_Rest {
	...
	public static function register_routes(): void {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'permission_callback' => array( __CLASS__, 'connect_permissions_check' ),
				'callback'            => array( __CLASS__, 'handle_init_bridge' ),
			)
		);
	...
	public static function send_cors_headers( $value ) {
		...
		$allow_origin = '';
		if ( self::is_allowed_chrome_extension_origin( $origin ) ) {
			$allow_origin = $origin;
		} elseif ( 'null' === $origin ) {
			$allow_origin = 'null';
		} else {
	...
	private static function is_allowed_chrome_extension_origin( string $origin ): bool {
		if ( ! preg_match( '#^chrome-extension://([a-p]{32})$#', $origin, $m ) ) {
			return false;
		}
		$extension_id = $m[1];
		if ( defined( 'IMPORTONBRIDGE_EXTENSION_ID' ) && IMPORTONBRIDGE_EXTENSION_ID ) {
			return $extension_id === IMPORTONBRIDGE_EXTENSION_ID;
		}
		return true;
	}
	...
	public static function debug_permissions_check( WP_REST_Request $request ): bool {
		...
		public static function handle_init_bridge( WP_REST_Request $request ): WP_REST_Response {
			nocache_headers();
			$user     = wp_get_current_user();
			$site_url = rtrim( home_url( '/' ), '/' );
